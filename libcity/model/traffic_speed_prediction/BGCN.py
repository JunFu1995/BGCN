...
import os 
def load_weight_matrix(file_path):
    '''
    ...
    W = 0
    try:
        W = scio.loadmat(file_path)['W']
        print(W.shape)
    except FileNotFoundError:
        print(f'ERROR: input file was not found in {file_path}.')
    return W 

def laplacian_normalization(A):
    """
    A = N, N 
    """
    D = A.sum(1) # B, N 
    ...
    D_1 = 1/D
    D_1 = torch.diag(D_1)
    A = torch.matmul(D_1, A)
    return A

def load_adj_mx(num_nodes):
    adj_path = ""
    if num_nodes == 307:
        adj_path = os.path.join('./mat/', 'pems04_map.mat')
    elif num_nodes == 358:
        adj_path = os.path.join('./mat/', 'pems03_map.mat')
    elif num_nodes == 883:
        adj_path = os.path.join('./mat/', 'pems07_map.mat')
    elif num_nodes == 170:
        adj_path = os.path.join('./mat/', 'pems08_map.mat')
    print(num_nodes, adj_path)
    A = load_weight_matrix(adj_path)
    A = laplacian_normalization(torch.tensor(A+np.eye(A.shape[0]))).numpy()
    return A
...
class GCN(nn.Module):
    ...
    def forward(self, x, support):
        ...
        x1 = self.mlp(self.nconv(x,support[0]))
        h = self.down(x) + x1
...
class BGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        if self.gcn_bool and self.addaptadj:
            self.PA = nn.Parameter(torch.ones(self.num_nodes, self.num_nodes)) # global 
            nn.init.constant_(self.PA, 1e-6)
            A = load_adj_mx(self.num_nodes)
            self.A = Variable(torch.from_numpy(A.astype(np.float32)).to(self.device), requires_grad=False)
            self.d = 0.5
            self.supports
        ...
        for b in range(self.blocks):
            ...
            for i in range(self.layers):
                # conv1d attention in pytorch 2.0 https://blog.csdn.net/weixin_44897685/article/details/131582684
    ...
    def forward(self, batch):
        ...
        new_supports = [F.dropout(self.A+self.PA, self.d, training=self.training)]
        # WaveNet layers
